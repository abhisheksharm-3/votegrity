...
export default function VoteList() {
  ...
  const { elections, handleJoinElection, isLoading } = useUserData()
  ...
  const filteredElections = elections?.filter(election => {
    if (!election?.title || !election?.category) return false;
    return (
  ...
  const onJoinElectionClick = async () => {
    ...
    try {
      setIsJoining(true)
      const result = await handleJoinElection(electionCode)

      if (result.success) {
        toast.success("Success", {
          description: "Successfully joined election!",
        })
        setElectionCode('')
      } else {
        toast.error("Error", {
          description: result.error || "Failed to join election",
        })
      }
    } catch (error) {
      toast.error("Error", {
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredElections.length > 0 ? (
                filteredElections.map(election => {
                  const status = getElectionStatus(String(election.startDate), String(election.endDate))
                  return (
